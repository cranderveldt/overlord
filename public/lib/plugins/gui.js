...
.defines(function () {
			...
			*/
			...
			,draw: function() {
				...
				for (var i = 0; i < ig.gui.elements.length; i++) {
					...
					// Pressed
					if(state == 'hover' && (ig.input.pressed('mouse1') || ig.input.pressed('mouse1'))) {
						state = 'active';
						if(ig.input.state('mouse1') && typeof ig.gui.elements[i].mouseDown == 'function')
            {
              ig.gui.elements[i].mouseDown.call(element);
						...
						if(ig.gui.isClicking) {
              ig.gui.isClicking = false;
							// Toggle (click)
							if(element.toggle){
	...
	ig.Game.inject({
		update: function() {
			this.parent();
      ig.gui.isClicking = ig.input.pressed("mouse1");
			// Initialize GUI
			if(ig.gui.initialized) return; else ig.gui.initialized = true;

			// Bind Do this in main game.
